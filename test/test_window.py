...
class TestIsWindowValid:
    ...
    @pytest.mark.parametrize(
        ...
        ids=['is-not-window', 'window-not-visible', 'empty-title-bar', 'zeroed-rect', 'cloaked-window'],
    )
    def test_basic_rejections(self, mocker: MockerFixture, mock_checks: dict, index: int, value):
    ...
    @pytest.mark.parametrize(
        ...
        ids=['no-state', 'not-state-invisible', 'state-invisible', 'state-includes-invisible'],
    )
    def test_titlebar_rejection(self, mocker: MockerFixture, mock_checks, state, expected):
