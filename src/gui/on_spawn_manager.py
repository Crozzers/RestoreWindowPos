...
class OnSpawnPage(wx.Panel):
    def __init__(self, parent: wx.Frame):
        ...
        self.settings = OnSpawnSettings(
            enabled=False, apply_lkp=True, apply_rules=True, ignore_children=True, capture_snapshot=True)
        ...
        ignore_children_opt = wx.CheckBox(
            self.panel, id=4, label='Ignore child windows')
        ...
        capture_snapshot_opt = wx.CheckBox(
            self.panel, id=5, label='Capture a new snapshot')
        ...
        # bind events
        for widget in (enable_opt, apply_lkp_opt, apply_rules_opt, ignore_children_opt, capture_snapshot_opt):
            widget.Bind(wx.EVT_CHECKBOX, self.on_setting)

        # place
        simple_box_sizer(
            self.panel,
            (header1, apply_lkp_opt, apply_rules_opt,
    ...
    def on_setting(self, event: wx.Event):
        ...
        if isinstance(widget, wx.CheckBox):
            key = {1: 'enabled', 2: 'apply_lkp', 3: 'apply_rules',
                   4: 'ignore_children', 5: 'capture_snapshot'}[event.Id]
